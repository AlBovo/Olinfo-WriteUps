...
private:
    ...
    void set_range(int node, int nl, int nr, int ql, int qr, long long x){
        ...
        seg[node].valuem = std::min(seg[node * 2].valuem, seg[node * 2 +1].valuem);
    }
...
public:
    Segtree(int sizep, const std::vector<long long>&a){
        n = sizep;
        size = 1 << (int)(std::ceil(std::log2(sizep)));
        node n;
        n.values = 0;
        ...
        for(int i=size-1; i>0; i--){
            ...
            seg[i].valuem = std::min(seg[i*2].valuem, seg[i*2+1].valuem);
        }
    }
...
Segtree seg(1, std::vector<long long>(1));
